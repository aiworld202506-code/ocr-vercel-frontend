...
async function waitForRecognition(jobId, headers) {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      emptyResult.textContent = hadNetworkIssue
        ? "正在识别，网络连接已恢复..."
        : data.status === "queued"
          ? "任务排队中，请稍候..."
          : "正在识别，请稍候...";
...
function parseRawJson(rawText) {
  ...
  try {
    ...
    return recoverTruncatedOcrJson(cleaned);
  }
}

function recoverTruncatedOcrJson(text) {
  const summaryMatch = text.match(/"summary"\s*:\s*"((?:\\.|[^"\\])*)"/);
  const linesStart = text.search(/"lines"\s*:\s*\[/);
  if (linesStart < 0) return null;

  const objectPattern = /\{\s*"line_number"\s*:\s*(\d+)\s*,\s*"text"\s*:\s*"((?:\\.|[^"\\])*)"(?:\s*,[\s\S]*?)?\s*\}/g;
  objectPattern.lastIndex = linesStart;
  const lines = [];
  let match;
  while ((match = objectPattern.exec(text)) !== null) {
    try {
      ...
      // Ignore only the incomplete final object.
    }
  }
  ...
  return {
    summary,
    lines,
    uncertain_items: ["模型输出被截断，已恢复截断前完整返回的文字行。"],
  };
}
